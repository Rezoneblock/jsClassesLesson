class Soldier {
    constructor(id, name, squad, rank) {
        this.id = id;
        this.name = name;
        this.squad = squad;
        this.rank = rank;
...
class Platoon {
    squads = [[], [], [], []]; // Все отделения во взводе
    _id = 0; // Уникальный идентификатор для каждого солдата.

    ranks = ['private', 'corporal', 'sergeant']; // Доступные звания для солдат
    ...
    addSoldier(name, squad, rank = this.ranks[0],  id = this._id + 1) { // Добавить солдата
        if (squad == 0) {
    ...
    findSoldierById(id) { // Найти солдата по его id
    ...
    removeSoldierById(id) { // Удалить солдата по его id
        for (const squad of this.squads) {
    ...
    promoteSoldier(id, newRank) { // Повысить солдата
    ...
    get allSoldiers() { // Вывести всех солдат из взвода
...
// console.log(platoon.findSoldier('3'));

// Удалить солдата по его id
// console.log(platoon.removeSoldierById(6));
// console.log(platoon);

// Повысить солдата
// platoon.promoteSoldier(4, 'sergeant');
// console.log(platoon);
